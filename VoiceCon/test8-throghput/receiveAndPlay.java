...
public class receiveAndPlay extends Thread{
   ...
   public receiveAndPlay( int receiveport,String sendIP ){
		...
		try{
			writer = new FileWriter(receiveport+"output.txt", true);
	...
	try(MulticastSocket receivesocket = new MulticastSocket( receiveport );){
		...
		int packetNum=0;
         ...
         for( ;;){
			...
			writer.write(System.currentTimeMillis() + " " +((Record.p).inNum)+"\r\n");
			noOfReceivedPackets++;
			//use 1000 packets to test performance
			if(noOfReceivedPackets>1000) {writer.close();System.exit(0);}
